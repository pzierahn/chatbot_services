...
func (client *Client) CreateUsage(ctx context.Context, usage Usage) error {
	_, err := client.conn.Exec(ctx, "INSERT INTO openai_usage (id, uid, model, input, output) VALUES ($1, $2, $3, $4, $5)",
...
func (client *Client) ReadUsage(ctx context.Context, id string) (Usage, error) {
	var usage Usage
	err := client.conn.QueryRow(ctx, "SELECT id, created_at, uid, model, input, output FROM openai_usage WHERE id = $1", id).
...
func (client *Client) UpdateUsage(ctx context.Context, usage Usage) error {
	_, err := client.conn.Exec(ctx, "UPDATE openai_usage SET uid = $2, model = $3, input = $4, output = $5 WHERE id = $1",
		usage.ID, usage.UID, usage.Model, usage.Input, usage.Output)
	return err
}
